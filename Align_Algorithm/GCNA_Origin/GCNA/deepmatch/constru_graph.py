...
def combin_graph(inptut_graph,ratio,sample_nodes,aline_file,out_graph,isdirected):
    ...
    # matches, matches_ms = get_matches(GC1, GC2)
    ...
    for i in edges_g2:
        ...
        if u not in nodes_match:
            if u in match_g2:
                ...
                u = tag(nodes_g2, u, len(nodes_g2), G.number_of_nodes())
        if v not in nodes_match:
            if v in match_g2:
                v = match_g1[match_g2.index(v)]
            else:
                v = tag(nodes_g2, v, len(nodes_g2), G.number_of_nodes())
    ...
    for i in range(G1_sample.number_of_nodes()):
        ...
        if i in match_g2:
            continue
        fb.writelines(str(i)+" "+str(tag(nodes_g2, i,len(nodes_g2),G.number_of_nodes()))+"\n")
    fb.close()
    const(G.number_of_nodes()*2-len(nodes_g2),isdirected,out_graph)
    return (G.number_of_nodes()*2-len(nodes_g2))


#isdirected
# graph = combin_graph("data/ppi.txt", 0.90, 500, "aline_true.txt", "combin_ppi.txt", False)
