...
from deepmatch.refinement import *
...
def get_matches(G1, G2):
    ...
    after_rep = time.time()
    print("deepwalk_匹配时间 %f seconds" % (after_rep - before_rep))
    for match in matches:
        if match[0] == match[1]:
            count += 1
        else:
            print("不匹配的点："+str(match[0])+" "+str(match[1]))
...
def main():
    ...
    parser.add_argument('--input1', default='data/blocat.txt',
						help='Input file of graph')

    parser.add_argument('--input2', default='data/blocat.txt',
                        help='Input file of graph')
