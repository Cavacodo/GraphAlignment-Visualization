...
def get_acc(source_outputs, target_outputs, test_dict = None, alphas=None, just_S = False):
    ...
    list_S_numpy = []
    accs = ""
    for i in range(0, len(source_outputs)-1):
...
def Laplacian_graph(A):
    ...
    for i in range(len(A)):
        ...
        """
        A[i, i] = 1
    A = torch.FloatTensor(A) #转成float型矩阵
    """
        torch.diag()取得是矩阵的对角线元素(如果仅为一行，则取每列的构建成一个n * n的矩阵):Tensor(3906,3906)
        torch.sum(A, 0)表示行的和，且返回的是Tensor(3906,)，即是一个行元素
            tensor([ 3.,  6., 32.,  ...,  2.,  2.,  2.])
        torch.sum(A, 0)**(-0.5)：Tensor(3906,)
            tensor([0.5774, 0.4082, 0.1768,  ..., 0.7071, 0.7071, 0.7071])
    """
    D_ = torch.diag(torch.sum(A, 0)**(-0.5)) #构建对角度矩阵，记住这个写法
    A_hat = torch.matmul(torch.matmul(D_,A),D_) #A_hat = D_ * A * D_
    A_hat = A_hat.float()
    """
        记录A_hat中的非零元素的索引，并进行转置，:Tensor(2,20234)
        即
        原来是：tensor([[   0,    0],
                [   0,    1],
                [   0,    2],
                ...
                [3905,   72],
                [3905, 3905]])
        t()操作后：tensor([[   0,    0,    0,  ..., 3904, 3905, 3905],
                [   0,    1,    2,  ..., 3904,   72, 3905]])
    ...
    """
       上一步计算出了索引，这一步是取索引当中的值：Tensor(20234,)
        tensor([0.3333, 0.2357, 0.1021,  ..., 0.5000, 0.2357, 0.5000])
    """
    ...
    """
        构建稀疏矩阵
            ...
            即最后的A_hat:
                tensor(
                    indices=tensor([[   0,    0,    0,  ..., 3904, 3905, 3905],
                                    [   0,    1,    2,  ..., 3904,   72, 3905]]),
