...
def get_khop_neighbors(graph, rep_method):
    ...
    for node in range(graph.N):
        neighbors = graph.G_adj.getrow(node).nonzero()[1].tolist() ###
    ...
    # pickle.dump(kneighbors_dict, open("kneighbor","wb"), protocol=2)

    # import pickle
    # kneighbors_dict = pickle.load(open("kneighbor","rb"))
    all_neighbors = {}
    for node in range(graph.N):
...
def get_features(graph, rep_method, verbose=True):
    ...
    # import cPickle as pickle
    # pickle.dump(khop_neighbors_nobfs, open("khop","wb"), protocol=2)

    # import pickle
    # khop_neighbors_nobfs = pickle.load(open("khop","rb"))
