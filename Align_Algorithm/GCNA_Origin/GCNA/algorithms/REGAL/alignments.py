...
def get_embedding_similarities(embed, embed2=None, sim_measure="cosine", num_top=None):
    ...
    if sim_measure == "cosine":
        ...
        similarity_matrix = sklearn.metrics.pairwise.euclidean_distances(np.around(embed, 2), np.around(embed2, 2))
...
def score_alignment_matrix(alignment_matrix, topk=None, topk_score_weighted=False, true_alignments=None):
    ...
    if topk is None:
        ...
        for node_index in range(n_nodes):
            try: # in case the number of src nodes isn't equals the number of trg nodes
                true_alignments[node_index]
            except:
                continue
        ...
        alignment_score /= float(min(alignment_matrix.shape))
