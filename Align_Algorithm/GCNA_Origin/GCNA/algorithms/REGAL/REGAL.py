...
class REGAL(NetworkAlignmentModel):
    ...
    def merge_source_target_graphs(self):
        ...
        #new_nodes_idxs1 = np.arange(1767)
        new_nodes_idxs2 = np.arange(len(G1.nodes()), len(G1.nodes()) + len(G2.nodes()))
        #new_nodes_idxs2 = np.arange(1767, 1767 + 1767)
        ...
        for link in res1["links"]:
            new_source_index = str(link["source"])
            new_target_index = str(link["target"])
            new_links.append({
    ...
    def align(self):
        ...
        self.source_emb.astype(np.float32)
        self.target_emb.astype(np.float32)
