...
class IONE(NetworkAlignmentModel):
    def __init__(self, source_dataset, target_dataset, gt_train, epochs=100, dim=100, seed=123, learning_rate=0.05):
        """
    ...
    def _save_data_as_ione_format(self):
        src_edgelist = list(self.source_dataset.G.edges())
        trg_edgelist = list(self.target_dataset.G.edges())
        src_nodes = list(self.source_dataset.G.nodes())
        trg_nodes = list(self.target_dataset.G.nodes())
